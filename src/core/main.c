...
struct option longopts[] = {{"interface", required_argument, NULL, 'i'},
...
void usage(int status) {
  ...
  printf(
      ...
      "-g, --force-gratuitous  Force the gratuitous flag to be set on all "
      "RREQ's.\n"
      "-h, --help              This information.\n"
      "-i, --interface         Network interfaces to attach to. Defaults to "
      "first\n"
      "                        wireless interface.\n"
      ...
      "-n, --n-hellos          Receive N hellos from host before treating as "
      "neighbor.\n"
      "-u, --unidir-hack       Detect and avoid unidirectional links "
      "(experimental).\n"
      ...
      "-R, --rate-limit        Toggle rate limiting of RREQs and RERRs "
      "(default ON).\n"
...
struct sockaddr_in *get_if_info(char *ifname, int type) {
  ...
  if (ioctl(skfd, type, &ifr) < 0) {
    alog(LOG_ERR, errno, __FUNCTION__, "Could not get address of %s ", ifname);
    close(skfd);
    return NULL;
  } else {
...
void host_init(char *ifname) {
  ...
  do {
    ...
    if (ioctl(if_sock, SIOCGIFINDEX, &ifreq) < 0) {
      alog(LOG_ERR, errno, __FUNCTION__, "Could not get index of %s", iface);
      close(if_sock);
      exit(-1);
    }
...
int main(int argc, char **argv) {
  ...
  while (1) {
    ...
    if ((n = pselect(nfds, &rfds, NULL, NULL, &timeout_spec, &origmask)) < 0) {
      if (errno != EINTR)
        alog(LOG_WARNING, errno, __FUNCTION__, "Failed select (main loop)");
      continue;
    }
